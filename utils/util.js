...
function testMobile(num) {
  console.log
  let myreg = /^(((13[0-9]{1})|(15[0-9]{1})|(18[0-9]{1})|(17[0-9]{1})|(16[0-9]{1})|(19[0-9]{1}))+\d{8})$/;
  if (num.length == 0) {
      wx.showToast({
          title: '手机号为空',
...
function sentRes(url, data, method, fn) {
  data = data || null;
  if (data == null) {
      let content = require('querystring').stringify(data);
  } else {
      let content = JSON.stringify(data); //json format
  }
